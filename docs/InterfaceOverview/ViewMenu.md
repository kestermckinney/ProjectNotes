...
When internal items are hidden, the **Internal** column and checkbox are removed from the view, so clients do not have a visual indicator that some items are hidden. Internal items will not be displayed in database search results found in the [File Menu](<../InterfaceOverview/FileMenu.md>). This is useful when presenting project information to clients — you can hide internal risks, issues, or meeting notes that are for your team only.
...
The [Filter Tool](<FilterTool.md>) provides a powerful interface to create filters on any column in a table. The filter panel appears at the bottom of the table view and allows you to define custom filter conditions.
...
The Filter Tool panel appears below the table, where you can define your filter conditions. See the [Filter Tool](<FilterTool.md>) documentation for detailed instructions on creating and using filters.
