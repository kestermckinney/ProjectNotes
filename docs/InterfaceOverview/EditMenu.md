...
**Record Editing**
Each page is design to only have one list view visible at a time. The toolbar and the **Edit** menu contain record editing commands that are enabled whenever an editable list view is visible.  When a record is copied, Project Notes will make sure no unique values are duplicated.  For example tracker items will get a new Item Number.  Copied meetings will not copy notes, but will copy the title and attendees to make creating a new meeting quicker.
