...
ItemDetailsPage::~ItemDetailsPage()
{
    ...
    if (m_itemDetailsDelegate)
        delete m_itemDetailsDelegate;
}

void ItemDetailsPage::openRecord(QVariant& recordId)
{
    setRecordId(recordId);

    // only select the records another event will be fired to open the window to show them
    global_DBObjects.actionitemsdetailsmodel()->setFilter(0, recordId.toString());
    global_DBObjects.actionitemsdetailsmodel()->refresh();

    global_DBObjects.trackeritemscommentsmodel()->setFilter(1, recordId.toString());
...
void ItemDetailsPage::setupModels( Ui::MainWindow *ui )
{
    ...
    if (m_itemDetailsDelegate == nullptr)
        m_itemDetailsDelegate = new ItemDetailsDelegate(this);

    m_mapperItemDetails->setItemDelegate(m_itemDetailsDelegate);
    ...
    ui->comboBoxType->addItems(DatabaseObjects::item_type);

    ui->comboBoxStatus->clear();
    ui->comboBoxStatus->addItems(DatabaseObjects::item_status);

    ui->comboBoxPriority->clear();
    ui->comboBoxPriority->addItems(DatabaseObjects::item_priority);
